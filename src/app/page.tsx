...
import { HostGlobe } from "@/components/three/host-globe";
...
export default function HomePage() {
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <section className="relative min-h-[85svh] flex flex-col items-center justify-center px-6">
        {/* 3D hero set-piece — replaces the old <video> background.
            Self-gates to null when WebGL is unsupported or reduced-motion is set. */}
        <HeroScene className="absolute inset-0 -z-10" />

        {/* Cinematic cream wash so copy stays legible over the 3D layer (matches old overlay) */}
        <div className="absolute inset-0 -z-10 pointer-events-none bg-gradient-to-b from-cream/15 via-cream/10 to-cream/70" />
        <Spotlight />

        {/* Content */}
        <div className="relative z-10 text-center">
          {/* Mascot — kept; floats via Reveal/3D ambience */}
          <Reveal as="div" className="relative mx-auto w-48 h-48 sm:w-56 sm:h-56 mb-6">
            <Image
          ...
          <Reveal as="div" delay={0.1}>
            ...
            <h1 className="text-4xl sm:text-5xl font-black leading-none mb-4">
              <span className="block text-navy mb-2">Welcome to the</span>
          ...
          {/* Countdown */}
          <Reveal as="div" delay={0.25} className="max-w-sm mx-auto mt-6">
            <CountdownTimer />
          </Reveal>
        </div>
      </section>

      {/* ═══ QUICK ACTIONS — premium TiltCards, big, tappable ═══ */}
      <section className="px-5 py-8 sm:py-12 relative z-20">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-4xl mx-auto">
          {quickActions.map((item, i) => (
            <Reveal key={item.href} as="div" delay={i * 0.08}>
              <Link
                href={item.href}
                className="block h-full rounded-2xl focus:outline-none focus-visible:ring-2 focus-visible:ring-gold active:scale-[0.96] transition-transform"
              >
                <TiltCard className="card p-5 h-full flex flex-col items-center gap-2 text-center">
      ...
      <section className="px-5 py-8 sm:py-12">
        <Reveal as="div" className="max-w-2xl mx-auto">
          <TiltCard className="card p-6 text-center">
            <Image
      ...
      {/* ═══ HOST CITIES — visible accessible city grid + decorative host globe ═══ */}
      <section className="relative px-5 py-8 sm:py-12">
        <Reveal as="div" className="text-center max-w-2xl mx-auto mb-6">
          <p className="text-navy/50 text-xs font-semibold uppercase tracking-[0.2em] mb-2">
            FIFA World Cup 2026
          </p>
          <h2 className="text-3xl sm:text-4xl font-black leading-tight">
            <GradientText>16 Cities. 3 Nations. One Cup.</GradientText>
          </h2>
          <p className="text-navy/50 text-sm mt-3">
            From coast to coast across North America — with Dallas right at the heart of it.
          </p>
        </Reveal>
        {/* HostGlobe renders a visible, accessible grid of all 16 host cities
            (the real content) plus a decorative WebGL globe when supported. */}
        <Reveal as="div" delay={0.1} className="max-w-3xl mx-auto">
          <HostGlobe className="w-full aspect-square sm:aspect-[16/10]" />
        </Reveal>
      </section>

      {/* ═══ SPONSOR INSTAGRAMS ═══ */}
      <section className="px-5 py-8 sm:py-12">
        <Reveal as="p" className="text-xs font-bold text-navy/40 uppercase tracking-[0.2em] mb-4 text-center">
          Follow Us
        </Reveal>
        <Reveal as="div" delay={0.05} className="flex items-center justify-center gap-6">
          {sponsorInstagrams.map((ig) => (
            <Magnetic key={ig.handle}>
              <a
                href={ig.href}
                target="_blank"
                rel="noopener noreferrer"
